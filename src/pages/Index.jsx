...
const Index = () => {
  ...
  // Trainer attendance stats for gym owners
  ...
  // Individual trainer attendance for trainers
  ...
  // Fetch attendance statistics for gym owners (MEMBER attendance)
  ...
  // Fetch TRAINER attendance statistics for gym owners
  const fetchTrainerAttendanceStats = async () => {
    if (!user || userRole !== 'gym-owner') {
      return;
    }
    
    try {
      const response = await authFetch('/attendance/trainers/stats');
      
      if (response.success || response.status === 'success') {
        const stats = response.data?.summary || {};
        setTrainerAttendanceStats({
          todayAttendance: stats.totalAttendanceToday || 0,
          weekAttendance: stats.totalAttendanceThisWeek || 0,
          monthAttendance: stats.totalAttendanceThisMonth || 0,
          totalAttendance: stats.totalAttendanceAllTime || 0
      ...
      console.error('Error fetching trainer attendance stats:', error);
      setTrainerAttendanceStats({
        todayAttendance: 0,
        weekAttendance: 0,
        monthAttendance: 0,
  ...
  // Fetch individual trainer's own attendance (for trainer role)
  const fetchMyAttendanceStats = async () => {
    ...
    try {
      // Get current trainer's attendance from user data
      const trainerAttendance = user.attendance || [];
      const now = new Date();
      const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
      const thisWeekStart = new Date(today.getTime() - (today.getDay() * 24 * 60 * 60 * 1000));
      const thisMonthStart = new Date(now.getFullYear(), now.getMonth(), 1);

      const todayCount = trainerAttendance.filter(record => {
        const recordDate = new Date(record.timestamp);
        return recordDate >= today;
      }).length;

      const weekCount = trainerAttendance.filter(record => {
        const recordDate = new Date(record.timestamp);
        return recordDate >= thisWeekStart;
      }).length;

      const monthCount = trainerAttendance.filter(record => {
        const recordDate = new Date(record.timestamp);
      ...
      setMyAttendanceStats({
        ...
        totalAttendance: trainerAttendance.length
      });
    } catch (error) {
      console.error('Error calculating trainer attendance stats:', error);
      setMyAttendanceStats({
        todayAttendance: 0,
        weekAttendance: 0,
        monthAttendance: 0,
  ...
  const handleOpenGate = async () => {
    ...
    if (result.success) {
      ...
      if (userRole === 'gym-owner') {
        ...
        fetchTrainerAttendanceStats();
      }
      // For trainers, refresh their stats and attendance
      if (userRole === 'trainer') {
        fetchTrainerStats();
    ...
    const loadDashboardData = async () => {
      ...
      try {
        if (userRole === 'super-admin') {
          ...
          // Fetch trainer attendance statistics
          await fetchTrainerAttendanceStats();
        } else if (userRole === 'trainer') {
          await fetchTrainerStats();
          // Fetch individual trainer's attendance
    ...
    switch (userRole) {
      ...
      case 'gym-owner':
        {
          ...
          return [
            ...
            { 
              ...
              value: isAttendanceLoading ? <LoadingIndicator /> : (
                <div className="flex flex-col">
                  <span>{trainerAttendanceStats.todayAttendance.toString()}</span>
                  <span className="text-xs text-gray-400">
                    {trainerAttendanceStats.weekAttendance} this week
                  </span>
                </div>
              ), 
              icon: Users, 
      ...
      case 'trainer':
        return [
          ...
          { 
            label: "My Attendance", 
            value: isTrainerStatsLoading ? <LoadingIndicator /> : (
              <div className="flex flex-col">
                <span>{myAttendanceStats.todayAttendance.toString()}</span>
                <span className="text-xs text-gray-400">
                  {myAttendanceStats.weekAttendance} this week
                </span>
              </div>
            ), 
            icon: Calendar, 
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {quickActions.map((action, index) => (
                  <Button
                    key={index}
                    variant="outline"
                    className="h-20 flex-col space-y-2 border-gray-600 text-white" // removed hover:bg-gray-700 and hover:text-white
