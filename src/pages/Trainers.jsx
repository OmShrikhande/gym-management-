...
const Trainers = () => {
  ...
  // Edit trainer: open Add form pre-filled
  const handleEditTrainer = (trainer) => {
    setFormData({
      ...
      specialization: trainer.specialization || 'Weight Training',
  ...
  // Delete trainer with confirmation and backend call
  const handleDeleteTrainer = async (trainer) => {
    if (!window.confirm(`Delete trainer ${trainer.name}? This action cannot be undone.`)) return;
    try {
      const res = await authFetch(`/users/${trainer.id}`, { method: 'DELETE' });
      if (res.success) {
        setMessage({ type: 'success', text: 'Trainer deleted successfully' });
        await fetchUsers();
      } else {
        setMessage({ type: 'error', text: res.message || 'Failed to delete trainer' });
      }
    } catch (err) {
      console.error('Delete trainer error:', err);
      setMessage({ type: 'error', text: 'An error occurred while deleting trainer' });
    }
  };
  ...
  const handleCreateTrainer = async (e) => {
    ...
    const isEdit = !!(selectedTrainer && showAddForm && formData.email === selectedTrainer.email);

    setFormSubmitting(true);
    setMessage({ type: 'info', text: isEdit ? 'Updating trainer...' : 'Creating trainer...' });
    ...
    try {
      if (isEdit) {
        ...
        const res = await authFetch(`/users/${selectedTrainer.id}`, {
          method: 'PATCH',
        ...
        if (res.success) {
          ...
          setMessage({ type: 'error', text: res.message || 'Failed to update trainer' });
        }
      } else {
        // Create new trainer
      ...
      setMessage({ type: 'error', text: 'An error occurred while saving the trainer' });
    } finally {
