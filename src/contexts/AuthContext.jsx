...
export const AuthProvider = ({ children }) => {
    ...
    try {
      ...
      if (userType === 'member') {
        ...
        requestBody = {
          ...
          // Manual amount and method wiring for MemberAgreement
          paymentMode: (userData.paymentMode || 'online'),
          paymentMethod: String(userData.paymentMethod || userData.paymentMode || 'online').toLowerCase(),
          // Persist paidAmount to User collection
          paidAmount: Number(userData.paidAmount ?? userData.paymentAmount ?? 0),
          agreedAmount: Number(userData.agreedAmount ?? 0)
        };
  ...
  // Delete member; supports OTP when required by backend
  // Pass otpCode when deleting a member as gym-owner
  const deleteMember = async (memberId, otpCode = null) => {
    setError(null);
    setIsLoading(true);
    ...
    try {
      ...
      // Helper to perform DELETE with optional body
      const doDelete = async (url) => {
        const hasBody = otpCode !== null && otpCode !== undefined && String(otpCode).trim() !== '';
        return fetch(url, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
          ...
          ...(hasBody ? { body: JSON.stringify({ otp: String(otpCode).trim() }) } : {})
        });
      };

      let response = await doDelete(`${API_URL}/auth/users/${memberId}`);

      if (!response.ok) {
        // Fallback to primary users route
        response = await doDelete(`${API_URL}/users/${memberId}`);
      }
      ...
      if (!response.ok) {
        // Detect OTP-related responses to guide UI
        const msg = data.message || '';
        const requiresOtp = /OTP/i.test(msg);
        setError(data.message || 'Member deletion failed');
        return { success: false, message: data.message || 'Member deletion failed', requiresOtp };
      }
  ...
  const requestMemberDeletionOtp = async (memberId) => {
    try {
      if (!memberId) return { success: false, message: 'memberId is required' };
      const response = await fetch(`${API_URL}/users/members/request-deletion-otp`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      console.error('requestMemberDeletionOtp error:', e);
  ...
  const updateCurrentUser = useCallback((updatedUser) => {
    ...
    setStorageItem(USER_STORAGE_KEY, { ...(user || {}), ...updatedUser });
  }, [user]);
  ...
  const authFetch = async (url, options = {}) => {
    ...
    try {
      // Request trace disabled to reduce console noise
