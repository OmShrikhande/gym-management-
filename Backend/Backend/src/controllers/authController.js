...
import { sendUserCreationReceipt } from '../services/emailService.js';
...
const createUserWithRole = async (req, res, role) => {
  try {
    ...
    // Also create a MemberAgreement record when a gym owner creates a member
    try {
      if (role === 'member' && req.user?.role === 'gym-owner') {
        const { default: MemberAgreement } = await import('../models/memberAgreementModel.js');

        // Accept payment fields and plan info from request
        const paymentMethod = (req.body.paymentMethod || req.body.payment_mode || '').toString().toLowerCase();
        const paidAmount = Number(req.body.paidAmount ?? req.body.amount ?? 0);
        const planId = req.body.planId || req.body.plan_id || undefined;
        const planType = req.body.planType || req.body.plan_type || undefined;
        const durationMonths = Number(req.body.durationMonths ?? req.body.membershipDuration ?? 0) || undefined;
        const membershipStartDate = req.body.membershipStartDate || newUser.membershipStartDate || undefined;
        const membershipEndDate = req.body.membershipEndDate || newUser.membershipEndDate || undefined;
        const assignedTrainer = newUser.assignedTrainer || req.body.assignedTrainer || undefined;
        const notes = req.body.notes || undefined;

        // Basic validation for required fields
        if (!paymentMethod || !['cash', 'online'].includes(paymentMethod)) {
          console.warn('[member-agreement] invalid or missing paymentMethod, expected cash|online');
        }
        if (!paidAmount || paidAmount < 0) {
          console.warn('[member-agreement] invalid or missing agreedAmount');
        }

        // Build snapshot from the created member document
        const memberSnapshot = {
          ...
          membershipDuration: newUser.membershipDuration,
        ...
        // Save agreement (keep existing payment saving elsewhere; this is additional)
        await MemberAgreement.create({
          member: newUser._id,
          gymOwner: req.user._id,
          paymentMethod,
          paidAmount: agreedAmount, // <-- fix here
          planId,
          planType,
          durationMonths,
          membershipStartDate,
      ...
      console.error('[member-agreement] failed to create agreement record:', e?.message);
      // Do not fail member creation if agreement creation fails
    }

    // Fire-and-forget: send user creation receipt email (does not block response)
    try {
      const gymName = req.user?.gymName || `${req.user?.name || 'Your Gym'}'s Gym`;
      let trainerName = undefined;
      if (role === 'member' && newUser.assignedTrainer) {
        try {
          const t = await User.findById(newUser.assignedTrainer).select('name');
          trainerName = t?.name;
        } catch (e) {
          console.warn('Could not fetch assigned trainer for receipt:', e?.message);
        }
      }
      ...
      const userData = {
        ...
        address: newUser.address,
        whatsapp: newUser.whatsapp,
        trainerFee: newUser.trainerFee
      };

      // Avoid awaiting to not slow down creation response
      // eslint-disable-next-line no-floating-promises
      sendUserCreationReceipt({
        to: newUser.email,
        userRole: role,
        ...
        trainerName,
        userData
      }).catch(err => console.warn('[email] user creation receipt failed:', err?.message));
    } catch (e) {
      console.warn('User creation receipt skipped:', e?.message);
    }
