...
export const verifyMembership = catchAsync(async (req, res, next) => {
  ...
  // Check membership status (derive from dates to avoid stale DB status)
  ...
  const derivedStatus = isGymOwnerAccessingOwnGym ? 'Active' : deriveMembershipStatus(member);

  if (!isGymOwnerAccessingOwnGym && derivedStatus !== 'Active') {
    // Best-effort: sync DB if derived expired but stored says otherwise
    if (derivedStatus === 'Expired' && member.membershipStatus !== 'Expired') {
      try { await User.findByIdAndUpdate(member._id, { membershipStatus: 'Expired' }, { new: false }); } catch {}
    }

    // Update door status to false (closed) for inactive/expired membership
    try {
      await realtimeDatabaseService.updateDoorStatus(gymOwnerId, false);
      await realtimeDatabaseService.logQRScanAttempt(
        memberId,
...
export const markAttendance = catchAsync(async (req, res, next) => {
  ...
  // Keep only current month's attendance records
  const monthStart = new Date(attendanceTimestamp.getFullYear(), attendanceTimestamp.getMonth(), 1);
  member.attendance = member.attendance.filter(r => {
    const ts = new Date(r.timestamp);
    return !isNaN(ts) && ts >= monthStart;
  });
...
// Mark attendance for a trainer (self or by gym owner)
export const markTrainerAttendance = catchAsync(async (req, res, next) => {
  const { gymOwnerId, trainerId } = req.body;

  // Validate input
  if (!gymOwnerId || !trainerId) {
    return next(new AppError('Gym owner ID and trainer ID are required', 400));
  }
  ...
  if (!trainer || trainer.role !== 'trainer') {
    return next(new AppError('Invalid trainer', 404));
  }

  // Permission checks
  if (req.user.role === 'trainer') {
    // Trainers can only mark their own attendance and only for their assigned gym
    if (req.user._id.toString() !== trainerId) {
      return next(new AppError('You can only mark your own attendance', 403));
    }
    if (req.user.gymId && req.user.gymId.toString() !== gymOwnerId) {
      return next(new AppError('You can only mark attendance for your assigned gym', 403));
    }
  } else if (req.user.role === 'gym-owner') {
    // Gym owners can only mark attendance for their own trainers
    if (trainer.gymId && trainer.gymId.toString() !== req.user._id.toString()) {
      return next(new AppError('You can only mark attendance for your trainers', 403));
    }
  } else if (req.user.role !== 'super-admin') {
    return next(new AppError('Insufficient permissions', 403));
  }

  // Record attendance with current server time
  trainer.attendance = trainer.attendance || [];
  const attendanceTimestamp = new Date();

  // Enforce trainer daily limit: max 2 entries per calendar day
  const todayStart = new Date(attendanceTimestamp.getFullYear(), attendanceTimestamp.getMonth(), attendanceTimestamp.getDate());
  const todayCount = trainer.attendance.filter(r => {
    const ts = new Date(r.timestamp);
  ...
  if (todayCount >= 2) {
    return res.status(429).json({
      status: 'error',
      message: 'Daily scan limit for trainer reached (2 per day).',
      data: { limit: 2, todayCount }
    });
  }

  // Keep only current month's attendance records
  const monthStart = new Date(attendanceTimestamp.getFullYear(), attendanceTimestamp.getMonth(), 1);
  trainer.attendance = trainer.attendance.filter(r => {
    const ts = new Date(r.timestamp);
    return !isNaN(ts) && ts >= monthStart;
  });

  trainer.attendance.push({ gymOwnerId, timestamp: attendanceTimestamp });
  await trainer.save({ validateBeforeSave: false });

  res.status(200).json({
    status: 'success',
    message: `Trainer attendance marked for ${gymOwner.gymName || gymOwner.name + "'s Gym"}`,
    data: {
      trainer: {
        id: trainer._id,
        name: trainer.name,
...
// Get attendance stats for all trainers of the authenticated gym owner
export const getAllTrainersAttendanceStats = catchAsync(async (req, res, next) => {
  const userId = req.user._id;

  // Only gym owners can view trainer-wide stats
  if (req.user.role !== 'gym-owner') {
    return next(new AppError('Only gym owners can view trainer attendance statistics', 403));
  }

  // Get all trainers of this gym
  const trainers = await User.find({
    role: 'trainer',
    $or: [{ createdBy: userId }, { gymId: userId }]
  ...
  res.status(200).json({
    ...
    data: {
      summary: {
        ...
        totalAttendanceThisMonth,
        totalAttendanceAllTime,
        averageAttendancePerTrainer: trainers.length > 0 ? Math.round(totalAttendanceAllTime / trainers.length) : 0
      },
